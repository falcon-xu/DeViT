...
from engine import train_1epoch_ens_freeze, evaluate_ens_disjoint
...
def main(args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats = train_1epoch_ens_freeze(model=model, ens_model=ens_model, criterion=distill_criterion,
                                              data_loader=data_loader_train, ens_optimizer=ens_optimizer,
                                              device=args.device, epoch=epoch, loss_scaler=loss_scaler, args=args,
                                              log=logger, ens_model_ema=ens_model_ema, mixup_fn=mixup_fn,
        ...
        if max_accuracy < test_stats["acc1"]:
            ...
            if args.output_dir and dist_utils.is_main_process():
                torch.save(model.state_dict(), os.path.join(output_dir, f"checkpoint.pth"))
...
if __name__ == '__main__':
    ...
    args.method = f'ens_freeze_{method[args.distillation_type]}_{args.loss}'
